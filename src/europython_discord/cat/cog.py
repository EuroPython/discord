...
class CatCog(commands.Cog):
    def __init__(
        ...
        bot: commands.Bot,
        config: CatConfig,
        client: CatClient | None = None,
    ) -> None:
        self.bot = bot
        self.config = config
        self._client = client or CatClient()
        self._last_usage_timestamp_by_user_id: OrderedDict[int, float] = OrderedDict()
        _logger.info("Cog 'Cat' has been initialized")
    ...
    async def cat_command(self, ctx: commands.Context) -> None:
        ...
        if (image_url := await self._client.fetch_random_cat()) is None:
            message = random.choice(self.config.error_messages)  # noqa: S311
            await ctx.send(message)
            return

        embed = discord.Embed()
        embed.description = "A random cat image from https://cataas.com"
