...
class DuckCog(commands.Cog):
    def __init__(
        ...
        bot: commands.Bot,
        config: DuckConfig,
        client: DuckClient | None = None,
    ) -> None:
        self.bot = bot
        self.config = config
        self._client = client or DuckClient()
        self._last_usage_timestamp_by_user_id: OrderedDict[int, float] = OrderedDict()
        _logger.info("Cog 'Duck' has been initialized")
    ...
    async def duck_command(self, ctx: commands.Context) -> None:
        ...
        if (image_url := await self._client.fetch_random_duck()) is None:
            message = random.choice(self.config.error_messages)  # noqa: S311
            await ctx.send(message)
            return

        embed = discord.Embed()
        embed.description = "A random duck image from https://random-d.uk"
